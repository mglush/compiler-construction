...
void CodeGenerator::visitAssignmentNode(AssignmentNode* node) {
    ...
    if (node->identifier_2) {
        ...
        std::cout << node->identifier_1->name << std::endl << std::endl << std::endl;

        std::cout << findVariableObjectName(this, this->currentClassName, node->identifier_1->name) << std::endl << std::endl << std::endl;

        // std::cout << getIndent(TAB_COUNTER) << "mov %eax, " << findVariableOffset(this, findVariableObjectName(this, this->currentClassName, node->identifier_2->name), node->identifier_2->name) << "(%ecx)";
...
void CodeGenerator::visitMemberAccessNode(MemberAccessNode* node) {
    // get the object offset, return it on the stack.
}
